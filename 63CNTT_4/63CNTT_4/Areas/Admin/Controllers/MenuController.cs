            ...
            if (!string.IsNullOrEmpty(form["ThemCategory"]))
            {
                ...
                if (!string.IsNullOrEmpty(form["nameCategory"]))
                {
                    ...
                    foreach (var row in listarr)
                    {
                        ...
                        // Trước khi chuyển đổi giá trị từ Session["UserID"], hãy kiểm tra xem nó có null hay không
                        if (Session["UserID"] != null)
                        {
                            // Chuyển đổi giá trị từ Session["UserID"] thành một số nguyên và gán cho menu.CreateBy
                            menu.CreateBy = Convert.ToInt32(Session["UserID"].ToString());
                        }
                        else
                        {
                            // Xử lý trường hợp khi Session["UserID"] là null
                            // Ví dụ: Hiển thị một thông báo hoặc thực hiện hành động khác theo ý bạn
                            TempData["message"] = new XMessage("danger", "Lỗi: UserID không khả dụng.");
                            // Có thể chuyển hướng hoặc xử lý ngay tại đây tùy thuộc vào yêu cầu của bạn
                        }
